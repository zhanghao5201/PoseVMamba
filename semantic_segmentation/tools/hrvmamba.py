...
class mamba_init:
    ...
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4):
        dt_proj = nn.Linear(dt_rank, d_inner, bias=True) #16,512
        # print("n",dt_rank)
        ## print("ssf",d_state, dt_rank, d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor)#ssf 1 16 512 1.0 random 0.001 0.1 0.0001 (256,512,1024,2048,)
    ...
    @classmethod
    def init_dt_A_D(cls, d_state, dt_rank, d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, k_group=4):#4个方向
...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        # print("888",dt_rank,d_model,ssm_ratio,d_inner)
        ...
        FORWARD_TYPES = dict(
            ...
            v05=partial(self.forward_corev2, force_fp32=False, no_einsum=True),  # selective_scan_backend="oflex", scan_mode="cross2d"   yeszh
            # ===============================
        ...
        if self.with_dconv:
            ...
            dw_kernel_size=dw_kernel_size, #dw_kernel_size,
            output_bias=False#dcn_output_bias,
        )
        ...
        if initialize in ["v0"]:
            ...
            )###yeszh torch.Size([4096, 1]) torch.Size([4096]) torch.Size([4, 1024, 32])
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if _scan_mode == 3:
            ...
            xs = xs.view(B, -1, L)
            dts = dts.contiguous().view(B, -1, L) #B,D,L
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        if self.with_dconv:
            if isinstance(self.conv2d, getattr(DCNv4, 'DCNv4')):
                # print("kofkpd")
                d1,d2,d3,d4=x.shape
                x=x.permute(0,2,3,1)
...
class SS2Dm0:
    ...
    def forward_corem0(
        ...
        **kwargs,
        ...
        L = H * W
        KR = K * R
        # print(B, H, W, RD, K,R,D,L,KR,"mmm",N)#tensor(1) tensor(64) tensor(48) tensor(32) tensor(4) tensor(2) tensor(16) tensor(3072) tensor(8) mmm  K:4 R: 2 D 16
        _scan_mode = dict(cross2d=0, unidi=1, bidi=2, cascade2d=3)[scan_mode]
    ...
    def forwardm0(self, x: torch.Tensor, **kwargs):
        ...
        if not self.disable_z:
            y = y * z
        out = self.dropout(self.out_proj(y)).permute(0, 3, 1, 2)
        return out
...
class HRVmambaBlock(BaseModule):
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.post_norm:
            ...
            x = torch.cat(x, dim=1)
            x = channel_shuffle(x, self.num_groups)
            x=self.drop_path(self.act0(x))+input
...
class HRVmamba(HRNet):
    ...
    def init_weights(self, pretrained=None):
        ...
        for m in self.modules():
            ...
            elif isinstance(m, (nn.LayerNorm, nn.GroupNorm, nn.SyncBatchNorm, nn.BatchNorm2d)):
                nn.init.constant_(m.bias, 0)
...
class Backbone_HRVMamba(HRVmamba):
    def __init__(self, out_indices=(0, 1, 2, 3), pretrained=None, **kwargs):
        kwargs.update()
        super().__init__(extra=kwargs["extra"], pretrained=pretrained )
        norm_layer="ln2d"
    ...
    def init_weights(self, ckpt=None):
        ...
        for name in ['outnorm0', 'outnorm1', 'outnorm2', 'outnorm3']:
            if hasattr(self, name):
                norm = getattr(self, name)
                if isinstance(norm, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.SyncBatchNorm)):
                    nn.init.constant_(norm.weight, 1.0)
                    nn.init.constant_(norm.bias, 0.0)
                    print(f"Initialized {name} weight=1, bias=0")    
