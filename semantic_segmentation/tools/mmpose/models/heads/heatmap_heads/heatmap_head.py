...
class HeatmapHead(BaseHead):
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tensor:
        ...
        """
        x = feats[-1]
        # print("a0",x.shape,self.deconv_layers,self.conv_layers,self.final_layer)#a0 torch.Size([32, 1024, 8, 6])
        # a0 torch.Size([64, 8, 96, 72]) Identity() Identity() Conv2d(8, 17, kernel_size=(1, 1), stride=(1, 1))

        x = self.deconv_layers(x)
        # print("a1",x.shape,self.conv_layers)#a1 torch.Size([32, 256, 64, 48])
        # a1 a3torch.Size([64, 8, 96, 72])  Identity()
        x = self.conv_layers(x)
        # print("a2",x.shape,self.final_layer)#torch.Size([32, 256, 64, 48])
        # a2 torch.Size([64, 8, 96, 72]) Conv2d(8, 17, kernel_size=(1, 1), stride=(1, 1))
        x = self.final_layer(x)
        # print("a3",x.shape)# a3 torch.Size([32, 17, 64, 48]) a3 torch.Size([64, 17, 96, 72])
