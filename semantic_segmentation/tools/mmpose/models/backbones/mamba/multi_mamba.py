...
# from mamba_ssm.ops.selective_scan_interface import MambaInnerFn as mamba_inner_fn_no_out_proj
...
class MultiScan(nn.Module):
    ...
    ALL_CHOICES = ('h', 'h_flip', 'v', 'v_flip', 'w2', 'w2_flip', 'w7', 'w7_flip','colw2', 'colw2_flip', 'colw7', 'colw7_flip','w9', 'w9_flip', 'colw9', 'colw9_flip')
    ...
    def scan(self, x, direction='h'):
        ...
        if len(x.shape) == 3:
            if direction == 'h':
                # print("h",x.shape,x.transpose(-2, -1).shape) #h torch.Size([256, 196, 768]) torch.Size([256, 768, 196])
                return x.transpose(-2, -1)
            elif direction == 'h_flip':
                # print("h_flip", x.shape, x.transpose(-2, -1).shape, x.transpose(-2, -1).flip([-1]).shape)
                return x.transpose(-2, -1).flip([-1])
            elif direction == 'v':
                # print("v",x.shape,rearrange(x, 'b (h w) d -> b d (w h)', h=H, w=W).shape)
                #v torch.Size([256, 196, 768]) torch.Size([256, 768, 196])
                return rearrange(x, 'b (h w) d -> b d (w h)', h=H, w=W)
            elif direction == 'v_flip':
                # print("v_flip", x.shape, rearrange(x, 'b (h w) d -> b d (w h)', h=H, w=W).flip([-1]).shape)
            ...
            elif direction.startswith('colw'):
                K = int(direction[4:].split('_')[0])
                flip = direction.endswith('flip')
...
class MultiMamba(nn.Module):
    def __init__(
        ...
        # print("kk",torch.arange(1, self.d_state + 1, dtype=torch.float32, device=device).shape,self.d_state)kk torch.Size([16]) 16
        '''new for search'''
        A = repeat(
        ...
        # print("f0",A.shape) # f0 torch.Size([384, 16])
        A_log = torch.log(A)  # Keep A_log in fp32
        # print("f1",A_log.shape) torch.Size([384, 16])f1
        for i in range(len(self.multi_scan.choices)):
            ...
            # print("f2",i, self.d_inner, self.d_inner, self.dt_rank ) # f2 0 384 384 12
            conv1d = nn.Conv1d(
            ...
            # print(D.shape,"ff5") torch.Size([384]) ff5
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        """
        # print(hidden_states.shape,"sdk0")#torch.Size([256, 196, 192]) sdk0
        xz = self.in_proj(hidden_states)
        # print(xz.shape,"sdk1") # torch.Size([256, 196, 768])

        xs = self.multi_scan.multi_scan(xz)  # [[BDL], [BDL], ...]
        # print(xs[0].shape,xs[1].shape,xs[2].shape,xs[3].shape,"sdk2") torch.Size([256, 768, 196]) torch.Size([256, 768, 196]) torch.Size([256, 768, 196]) torch.Size([256, 768, 196]) sdk2
        outs = []
        for i, xz in enumerate(xs):
            ...
            # print(xz.shape,"lll")#torch.Size([256, 768, 196]) lll
