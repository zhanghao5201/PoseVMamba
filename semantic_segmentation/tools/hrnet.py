...
from resnet import BasicBlock, Bottleneck, get_expansion,ResLayer
...
class HRFuseScales(BaseModule):
    ...
    def forward(self, x):
        # print("ss",len(x),self.in_channels)ss 4 (80, 160, 320, 640)
        # op00 torch.Size([1, 80, 128, 128]) torch.Size([1, 160, 64, 64]) torch.Size([1, 320, 32, 32]) torch.Size([1, 640, 16, 16])
        assert isinstance(x, list) and len(x) == len(self.in_channels)
        out=[]
        feat = self.increase_layers[0](x[0])
        out.append(feat)
        # print("op1",feat.shape)#op1 torch.Size([1, 128, 128, 128])
        for i in range(len(self.downsample_layers)):
            ...
            if i !=2:
                ...
                out.append(self.final_layer(feat))
            # print("ff",feat.shape,i)
        # print("op2",self.final_layer(feat).shape)
        return out
            # print("op",feat.shape)
        # print("op2",self.final_layer(feat).shape)op2 torch.Size([1, 2048, 16, 16])
        # return self.final_layer(feat),out
        ...
        for i in range(len(self.fuse_layers)):
            ...
            for j in range(self.num_branches):
                if i == j:
                    ...
                    # print("aa",self.fuse_layers[i][j](x[j]).shape)
...
class HRNet(BaseBackbone):
    ...
    def __init__(
        ...
        self.neck= HRFuseScales(in_channels=self.stage4_cfg['num_channels'])
    ...
    def forward(self, x):
        """Forward function."""
        # print("a0",x.shape)a0 torch.Size([256, 3, 224, 224])
        x = self.conv1(x)
        x = self.norm1(x)
        x = self.relu(x)
        x = self.conv2(x)
        ...
        x = self.relu(x)
        x = self.layer1(x)
        # print("a20",x.shape) a20 torch.Size([256, 256, 56, 56])
        ...
        y=self.neck(y_list)
        y = torch.flatten(self.avgpool(y), 1)
        y=self.head(y)        
        # print(y.shape,"oo")#torch.Size([128, 2048, 7, 7]) oo
        # print(y_list[3].shape,"090") #torch.Size([128, 32, 56, 56]) 090 torch.Size([128, 256, 7, 7]) 090
