...
def parse_option():
    ...
    parser.add_argument('--save-ckpt-num', default=3, type=int)
    parser.add_argument(
        '--use-zero',
        action='store_true',
...
def main(config):
    ...
    if config.TRAIN.EMA.ENABLE:
        ...
        if config.MODEL.RESUME:
            max_ema_accuracy=load_ema_checkpoint(config, model_ema, logger)
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0
            ...
            save_checkpoint(config,
                            ...
                            loss_scaler,
                            logger,
                            max_accuracy_ema=max_ema_accuracy,
                            model_ema=model_ema)
