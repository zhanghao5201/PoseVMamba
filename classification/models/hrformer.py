...
class HRFormerBlock(BaseModule):
    ...
    def forward(self, x):
        """Forward function."""
        # print(x.shape,"f0")#torch.Size([32, 156, 32, 24]) f0
        B, C, H, W = x.size()
        # Attention
...
class Backbone_HRFormer(HRFormer):
    def __init__(self, out_indices=(0, 1, 2, 3), init_cfg=[
        ...
        # print(num_channels,"kll")#(80, 160, 320, 640) kll
        # pdb.set_trace()
    ...
    def forward(self, x):
        ...
        # print("ak",len(y_list),self.out_indices)
        outs = []
        for i in range(len(self.out_indices)):
            ...
            # print(norm_layer,"jj")
            # print(out.shape,i,"ak")            
            outs.append(out.contiguous())

        # print(len(outs),outs[0].shape,outs[1].shape,outs[2].shape,outs[3].shape,"mmm")
        # 4 torch.Size([2, 80, 128, 128]) torch.Size([2, 160, 64, 64]) torch.Size([2, 320, 32, 32]) torch.Size([2, 640, 16, 16]) mmm
        return outs