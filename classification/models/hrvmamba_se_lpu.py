...
class Backbone_HRVMamba(HRVmamba_se_lpu):
    def __init__(self, out_indices=(0, 1, 2, 3), init_cfg=[
            dict(type='Normal', std=0.001, layer=['Conv2d']),
            dict(type='Constant', val=1, layer=['_BatchNorm', 'GroupNorm'])
        ], **kwargs):
        kwargs.update()
        super().__init__(init_cfg=init_cfg, in_channels=kwargs["in_channels"], norm_cfg =kwargs["norm_cfg"],extra=kwargs["extra"] )
        norm_layer="ln2d"
        ...
        if init_cfg is not None:
            self.load_pretrained(ckpt=init_cfg["checkpoint"])
    def load_pretrained(self, ckpt=None):
        if ckpt is None:
            return        
        try:
