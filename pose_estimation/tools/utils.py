...
def get_dataset(root="./val", img_size=(224,224), ret="", crop=True, single_image=False):
...
class AttnMamba:
    ...
    def attnmap_mamba(regs, mode="CB", ret="all", absnorm=0, scale=1, verbose=False, device=None):
        ...
        # attn1 = attns[:, 1, :].view(-1, H, W, H, W).contiguous().view(B, -1, L, L)

        attn1 = attns[:, 1, :].view(-1, W, H, W, H).permute(0, 2, 1, 4, 3).contiguous().view(B, -1, L, L)
        ...
        attn3 = attn3.view(-1, W, H, W, H).permute(0, 2, 1, 4, 3).contiguous().view(B, -1, L, L)
    ...
    @classmethod
    @torch.no_grad()
    def get_attnmap_mamba(cls, ss2ds, stage=-1, mode="", verbose=False, raw_attn=False, scale=1, device=None):
        ...
        if raw_attn:
            ss2d = ss2ds if not isinstance(ss2ds, list) else ss2ds[stage]
...
#     @staticmethod
#     def checkpostfix(tag, value):
#         # print(tag,"op",value[-len(tag):],value)
#         ret = value[-len(tag):] == tag
#         if ret:
#             value = value[:-len(tag)]
#         return ret, value
...
#         As, Bs, Cs, Ds = -torch.exp(regs["A_logs"].to(torch.float32)), regs["Bs"], regs["Cs"], regs["Ds"] #cs torch.Size([1, 4, 1, 196]) Ds torch.Size([4096])
#         #As: D,N  BS: B,K,N,L  Cs:B,K,N,L Ds:D(k*c)
#         us, dts, delta_bias = regs["us"], regs["dts"], regs["delta_bias"] # torch.Size([1, 4096, 196]) torch.Size([1, 4096, 196])  torch.Size([4096] 
#         #us: B,D,L dts: B,D,L delta_bias:D
#         ys, oy = regs["ys"], regs["y"] #ys:
#         # print("op",ys.shape, oy.shape)#op torch.Size([1, 4, 1024, 14, 14]) torch.Size([1, 1024, 196]) #zh op torch.Size([1, 4, 64, 64, 48]) torch.Size([1, 64, 3072])
#         H, W = regs["H"], regs["W"]
#         # print(H,W,"mm") #64 48 mm
#         # print(As.shape, Bs.shape, Cs.shape, Ds.shape, us.shape, dts.shape, delta_bias.shape,"oplk")#torch.Size([4096, 1]) torch.Size([1, 4, 1, 196]) torch.Size([1, 4, 1, 196]) torch.Size([4096]) torch.Size([1, 4096, 196]) torch.Size([1, 4096, 196]) torch.Size([4096]) oplk
#         B, G, N, L = Bs.shape #torch.Size([1, 4, 1, 196])  # torch.Size([1, 256, 28, 28]) torch.Size([1, 512, 14, 14]) torch.Size([1, 1024, 7, 7]) torch.Size([1, 1024, 7, 7])
#         GD, N = As.shape #torch.Size([4096, 1])
#         D = GD // G
#         # H, W = int(math.sqrt(L)), int(math.sqrt(L))
#         #  64 48 64 55 55 kk
#         # print(regs["H"], regs["W"],D,H,W,"kk") #ll 3072 torch.Size([3072, 3072]) torch.Size([1, 256, 3072]) 64 48 64 55 55 kk
#         if device is not None:
#             As, Bs, Cs, Ds, us, dts, delta_bias, ys, oy = As.to(device), Bs.to(device), Cs.to(device), Ds.to(device), us.to(device), dts.to(device), delta_bias.to(device), ys.to(device), oy.to(device)

#         mask = torch.tril(dts.new_ones((L, L)))
#         # print("ll",L, mask.shape,dts.shape) # ll torch.Size([196, 196]) torch.Size([1, 4096, 196])
#         dts = torch.nn.functional.softplus(dts + delta_bias[:, None]).view(B, G, D, L)
#         dw_logs = As.view(G, D, N)[None, :, :, None] * dts[:,:,:,None,:] # (B, G, D, N, L)
#         # print("jj",dw_logs.shape,torch.cumsum(dw_logs, dim=-1).shape)#torch.cumsum() 是 PyTorch 中一个非常常用的函数,它用于计算输入张量沿给定维度的累积和 jj torch.Size([1, 4, 1024, 1, 196]) torch.Size([1, 4, 1024, 1, 196])
#         ws = torch.cumsum(dw_logs, dim=-1).exp()
...
#         _Qs, _Ks = Qs.view(-1, N, L), Ks.view(-1, N, L)
#         attns = (_Qs.transpose(1, 2) @ _Ks).view(B, G, -1, L, L)
#         # print(attns.shape,mask.shape,"kl",attns.mean(dim=2).shape)#torch.Size([1, 4, 1024, 196, 196]) torch.Size([196, 196]) kl torch.Size([1, 4, 196, 196])
#         attns = attns.mean(dim=2) * mask
#         # print(attns.shape,"kl2") #torch.Size([1, 4, 196, 196]) kl2
#         # print("oo",attns.shape,"op")oo torch.Size([1, 4, 3072, 3072]) op
#         # pdb.set_trace()
...
#     @torch.no_grad()
#     def get_attnmap_mamba(cls, ss2ds, stage=-1, mode="", verbose=False, raw_attn=False, scale=1, device=None):
#         mode1 = mode.split("_")[-1] #["CB", "CwBw", "ww"]
#         mode = mode[:-(len(mode1) + 1)] #"a0", "a1", "a2", "a3", "all", "nall"
...
#         # print(mode)
#         tag, mode = cls.checkpostfix("_norm", mode)
#         # print("uu",tag,absnorm,mode)
#         absnorm = 1 if tag else absnorm
#         # print("o0",mode,absnorm)#1

#         if raw_attn:
#             regs = getattr(ss2ds[stage], "__data__")
#             # print(regs,"mm")
#             attn, H, W = cls.attnmap_mamba(regs, mode=mode1, ret=mode, absnorm=absnorm, verbose=verbose, scale=scale)
#             return attn

#         allrolattn = None
#         # print("03",len(ss2ds[stage]))
#         # for k in range(len(ss2ds[stage])):
#         regs = getattr(ss2ds[stage], "__data__")
#         attn, H, W = cls.attnmap_mamba(regs, mode=mode1, ret=mode, absnorm=absnorm, verbose=verbose, scale=scale)
#         L = H * W
