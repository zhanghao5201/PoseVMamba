...
def local_scan(x, w=7, H=14, W=14, flip=False, column_first=False):
    ...
    if column_first:
        ...
        #klkl torch.Size([256, 14, 14, 768]) torch.Size([256, 7, 2, 7, 2, 768]) torch.Size([256, 768, 7, 7, 2, 2])
        # print("klkl",x.shape,x.view(B, Hg, w, Wg, w, C).shape,x.view(B, Hg, w, Wg, w, C).permute(0, 5, 1, 3, 2, 4).shape)
        # print("nn",x.view(B, Hg, w, Wg, w, C).permute(0, 5, 1, 3, 2, 4).reshape(B, C, -1).shape,"ss")
        # nn torch.Size([256, 768, 196]) ss
...
def local_reverse(x, w=7, H=14, W=14, flip=False, column_first=False):
    ...
    B, C, L = x.shape
    Hg, Wg = math.ceil(H / w), math.ceil(W / w)
    # print("nnnmmm",w,flip,column_first)
    if flip:
        x = x.flip([-1])
    if H % w != 0 or W % w != 0:
        if column_first:
